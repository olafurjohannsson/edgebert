...
pub fn run_gpu_apply_mask(
    ...
    struct MaskUniforms { batch_size: u32, num_heads: u32, seq_len: u32, _padding: u32 }
    ...
    let uniforms = MaskUniforms { batch_size, num_heads, seq_len, _padding: 0 };
    let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("Apply Mask Uniforms"), contents: bytemuck::cast_slice(&[uniforms]), usage: wgpu::BufferUsages::UNIFORM,
    });

    let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
        label: Some("Apply Mask Bind Group"), layout: &bind_group_layout,
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: uniform_buffer.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 1, resource: scores.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 2, resource: mask.as_entire_binding() },
        ],
    });
    
    let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Apply Mask Compute Pass"), timestamp_writes: None });
