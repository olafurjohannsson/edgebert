...
use ndarray::{Array, Array1, Array2, s, Array3};
...
#[tokio::test]
    async fn test_apply_mask_correctness() -> Result<()> {
        ...
        run_gpu_apply_mask(&context, &mut encoder, &scores_gpu, &mask_gpu, batch_size as u32, num_heads as u32, seq_len as u32);
        context.queue.submit(std::iter::once(encoder.finish()));
        
        let gpu_result_array = read_buffer_to_ndarray(&context, &scores_gpu, (batch_size * num_heads, seq_len, seq_len)).await?;
