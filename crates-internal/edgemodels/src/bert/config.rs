...
// === BERT Base Config (TensorFlow-style with "bert." prefix and gamma/beta) ===

#[derive(Clone, Debug, Deserialize)]
pub struct BertBaseConfig {
    pub hidden_size: usize,
    pub num_attention_heads: usize,
    pub num_hidden_layers: usize,
    pub layer_norm_eps: f32,
...
impl ModelConfig for BertBaseConfig {}

impl TransformerConfig for BertBaseConfig {
    fn hidden_size(&self) -> usize {
        self.hidden_size
    }
    fn num_attention_heads(&self) -> usize {
...
impl EncoderArchitecture for BertBaseConfig {
    ...
    fn get_attention_names(&self, i: usize) -> LayerAttentionNames {
        let prefix = format!("bert.encoder.layer.{}.attention", i);
        LayerAttentionNames {
            q_weight: format!("{}.self.query.weight", prefix),
            q_bias: format!("{}.self.query.bias", prefix),
            k_weight: format!("{}.self.key.weight", prefix),
            ...
            output_weight: format!("{}.output.dense.weight", prefix),
            output_bias: format!("{}.output.dense.bias", prefix),
            norm_weight: format!("{}.output.LayerNorm.gamma", prefix),
            norm_bias: format!("{}.output.LayerNorm.beta", prefix),
        }
    }
