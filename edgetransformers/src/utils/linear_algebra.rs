...
#[inline(always)]
pub fn matmul_4d_simple(a: &Array4<f32>, b: &Array4<f32>) -> Array4<f32> {
    ...
    let (batch_size, num_heads, seq_len_q, _d_k) = a.dim();
    let seq_len_k = b.shape()[3];
    let mut output = Array4::<f32>::zeros((batch_size, num_heads, seq_len_q, seq_len_k));

    for b_idx in 0..batch_size {
        for h_idx in 0..num_heads {
            let a_slice = a.slice(s![b_idx, h_idx, .., ..]);
            let b_slice = b.slice(s![b_idx, h_idx, .., ..]);
            
            // Force each slice into a new, clean, standard-layout array before the dot product.
            let a_contiguous = a_slice.as_standard_layout().to_owned();
            let b_contiguous = b_slice.as_standard_layout().to_owned();

            let result = a_contiguous.dot(&b_contiguous);
            output.slice_mut(s![b_idx, h_idx, .., ..]).assign(&result);
        }
    }
    output
}
