//! Canonical composition pairs, generated from the Unicode database.
//!
//! `(base, combining) -> composed`, sorted so it can be binary searched.
//! Covers the range below U+2500, which is every Latin, Greek and Cyrillic
//! letter with a canonical decomposition: enough for captions in European
//! languages, which is what this is for. Regenerate with:
//!
//! ```text
//! python3 -c "import unicodedata; ..."  # see bench/clip_reference.py
//! ```

/// 870 pairs, sorted by `(base, combining)`.
pub(crate) static CANONICAL_COMPOSITIONS: [(u32, u32, u32); 870] = [
